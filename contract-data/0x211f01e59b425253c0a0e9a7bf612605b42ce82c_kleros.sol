...
contract Controlled {
    ...
    modifier onlyController {require(msg.sender == controller);
        _;}
...
contract BlockHashRNG is RNG {
    ...
    function saveRN(uint _block) public {
        ...
        if (randomNumber[_block] != 0) {// If the number is set.
            ...
            // Note that the use of send is on purpose as we don't want to block in case msg.sender has a fallback issue.
...
contract BlockHashRNGFallback is BlockHashRNG {
    ...
    function saveRN(uint _block) public {
        ...
        if (randomNumber[_block] != 0) {// If the random number is set.
            ...
            msg.sender.send(rewardToSend);
            // Note that the use of send is on purpose as we don't want to block in case the msg.sender has a fallback issue.
...
contract Arbitrable {
    ...
    modifier onlyArbitrator {require(msg.sender == address(arbitrator));
        _;}
...
contract Arbitrator {
    ...
    modifier requireArbitrationFee(bytes _extraData) {require(msg.value >= arbitrationCost(_extraData));
        _;}

    modifier requireAppealFee(uint _disputeID, bytes _extraData) {require(msg.value >= appealCost(_disputeID, _extraData));
        _;}
...
contract Kleros is Arbitrator, ApproveAndCallFallBack {
    ...
    modifier onlyBy(address _account) {require(msg.sender == _account);
        _;}

    modifier onlyDuring(Period _period) {require(period == _period);
        _;}
    ...
    function voteRuling(uint _disputeID, uint _ruling, uint[] _draws) public onlyDuring(Period.Vote) {
        ...
        // Make sure juror hasn't voted yet.
    ...
    function penalizeInactiveJuror(address _jurorAddress, uint _disputeID, uint[] _draws) public {
        ...
        // Verify the juror hasn't voted.

        dispute.lastSessionVote[_jurorAddress] = session;
        // Update last session to avoid penalizing multiple times.
    ...
    function oneShotTokenRepartition(uint _disputeID) public onlyDuring(Period.Execution) {
        ...
        for (uint i = 0; i <= dispute.appeals; ++i) {
            ...
            if (winningChoice != 0 || (dispute.voteCounter[dispute.appeals].voteCount[0] == dispute.voteCounter[dispute.appeals].winningCount)) {
                ...
                if (nbCoherent == 0) {// No one was coherent at this stage. Give the tokens to the governor.
                    ...
                    // Note that few fractions of tokens can be lost but due to the high amount of decimals we don't care.
            ...
            for (j = 0; j < dispute.votes[i].length; ++j) {
                ...
                // Note that it can't underflow due to amountShift not changing between vote and redistribution.
        ...
        // Since it was solved in one shot, go directly to the executable step.
    ...
    function multipleShotTokenRepartition(uint _disputeID, uint _maxIterations) public onlyDuring(Period.Execution) {
        ...
        // Total votes we have repartitioned this iteration.
    ...
    function validDraws(address _jurorAddress, uint _disputeID, uint[] _draws) public view returns (bool valid) {
        ...
        // Make sure that the tokens were deposited for this session.

        if (dispute.session + dispute.appeals != session) return false;
        // Make sure there is currently a dispute.

        if (period <= Period.Draw) return false;
        // Make sure that jurors are already drawn.

        for (uint i = 0; i < _draws.length; ++i) {

            if (_draws[i] <= draw) return false;
            // Make sure that draws are always increasing to avoid someone inputing the same multiple times.
            ...
            // Random position on the segment for draw.
