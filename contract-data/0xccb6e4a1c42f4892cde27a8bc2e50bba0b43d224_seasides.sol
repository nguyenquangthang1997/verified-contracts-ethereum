...
contract OnChainSeasides is ERC721, ERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 tokenId) pure public override(ERC721) returns (string memory) {
        ...
        params[0] = 1 + ((rand / 10) % 8);
        // ship

        params[1] = 1 + (rand / 100) % 2;
        // dir

        params[2] = 1 + ((rand / 10000) % 37);
        // pallette

        params[3] = 1 + ((rand / 1000000) % 8);
        // mounts

        params[4] = 1 + ((rand / 100000000) % 6);
        // grass

        params[5] = 1 + ((rand / 1000000000) % 4);
        // palm

        params[6] = 1 + ((rand / 10000000000) % 7);
        // ground

        params[7] = 1 + ((rand / 100000000000) % 4);
        // sun
        ...
        parts[0] = '<svg width="1000px" height="1000px" viewBox="0 0 1000 1000" version="1.1" xmlns="http://www.w3.org/2000/svg"> <linearGradient id="SkyGradient" gradientUnits="userSpaceOnUse" x1="500.001" y1="999.8105" x2="500.0009" y2="4.882813e-004"> <stop offset="0.5604" style="stop-color:#';
        // 2

        parts[1] = '"/> <stop offset="1" style="stop-color:#';
        // 3

        parts[2] = '"/> </linearGradient> <rect x="0.001" fill="url(#SkyGradient)" width="1000" height="999.811"/> <polygon opacity="0.15" fill="#';
        // 3

        parts[3] = string(abi.encodePacked('" points="', mount2[params[3] - 1], '"/> <polygon opacity="0.1" fill="#'));
        // 3

        parts[4] = string(abi.encodePacked('" points="', mount1[params[3] - 1], '"/> <rect x="0" y="478" opacity="0.2" fill="#'));
        // 3

        parts[5] = '" width="1000" height="734.531"/> <rect x="0" y="563.156" opacity="0.3" fill="#';
        // 3

        parts[6] = '" width="1000" height="649.315"/> <g> <path xmlns="http://www.w3.org/2000/svg" opacity="0.55" fill="#';
        // 3

        parts[7] = '" d="M8087,687c-158,0-320-3.15-469-3 c-293,0-616,10-701,10c-261,0-600-17-809-17 c-118,0-246,11-376,11c-158,0-320-10-469-10 c-293,0-379,10-574,10c-195,0-331-11-540-11 c-118,0-246,11-376,11c-158,0-320-10-469-10 c-293,0-616,17-701,17c-261,0-600-12-809-12 c-118,0-246,12-376,12c-103,0-263-9-469-9 c-92,0-181,2-260,2c-171,0-304,0-362,0c-261,0-330-0-330-0 v525l9053-6V688C9039,688,8217,687,8087,687z"/> <animateMotion path="M 0 0 L -8050 20 Z" dur="70s" repeatCount="indefinite" /> </g> <g> <path xmlns="http://www.w3.org/2000/svg" fill="#';
        // 3

        parts[8] = '" d="M8097,846c-158,0-319-7-470-7c-285,0-443,20-651,20 c-172,0-353-5-449-9c-101-4-247-20-413-20c-116,0-243,26-373,26 c-158,0-320-31-471-31c-285,0-352,36-560,36c-172,0-390-31-556-31 c-116,0-243,26-373,26c-158,0-320-31-471-31c-285,0-442,35-650,35 c-172,0-353-5-449-9c-101-4-247-20-413-20c-116,0-245,25-375,25 c-158,0-322-13-474-13c-107,0-197,2-277,3c-133,1-243,0-372,0 c-172,0-308-0-308-0v364h9053V846C9038,846,8227,846,8097,846z"/> <animateMotion path="M 0 0 L -8050 40 Z" dur="70s" repeatCount="indefinite" /> </g> <g> <polygon fill="#';
        // 3

        parts[9] = string(abi.encodePacked('" points="', getShip(params[0], params[1], 0), '"/> <polygon opacity="0.2" fill="#'));
        // 3

        parts[10] = string(abi.encodePacked('" points="', getShip(params[0], params[1], 1), '"/> <animateMotion path="m 0 0 h ', (params[1] == 1 ? '' : '-'), '5000" dur="1500s" repeatCount="indefinite" /> </g> <radialGradient id="SunGradient" ', getSun(params[7] * 2 - 2), ' gradientUnits="userSpaceOnUse"> <stop offset="0.7604" style="stop-color:#'));
        // 1

        parts[11] = '"/> <stop offset="0.9812" style="stop-color:#';
        // 2

        parts[12] = '"/> </radialGradient> <circle opacity="0.1" fill="#';
        // 2

        parts[13] = string(abi.encodePacked('" ', getSun(params[7] * 2 - 1), '/> <circle fill="url(#SunGradient)" ', getSun(params[7] * 2 - 2), '/> <g> <polygon fill="#'));
        // 4

        parts[14] = string(abi.encodePacked('" points="', getGrass(params[4] / 3 + 1, params[4] % 2), '"/> <animateMotion path="M 0 0 H 10 Z" dur="4s" repeatCount="indefinite" /> </g> <g> <path fill="#', colors[3], '" d="', getPalm(params[5]), '"/> <animateMotion path="M 0 0 H 15 Z" dur="5s" repeatCount="indefinite"/> </g> <polygon fill="#'));
        // 4
