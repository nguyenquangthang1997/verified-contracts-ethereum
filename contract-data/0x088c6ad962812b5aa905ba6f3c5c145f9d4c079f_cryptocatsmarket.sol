...
contract CryptoCatsMarket {
    ...
    function CryptoCatsMarket() payable {
        ...
        owner = msg.sender;
        // Set contract creation sender as owner

        _totalSupply = 625;
        // Set total supply

        catsRemainingToAssign = _totalSupply;
        // Initialise cats remaining to total supply amount
        ...
        // Register release 0 getting to 12 cats

        releaseCatIndexUpperBound.push(189);
        // Register release 1 getting to 189 cats

        releaseCatIndexUpperBound.push(_totalSupply);
        // Register release 2 getting to 625 cats



        catReleaseToPrice[0] = 0;
        // Set price for release 0

        catReleaseToPrice[1] = 0;
        // Set price for release 1

        catReleaseToPrice[2] = 80000000000000000;
        // Set price for release 2 to Wei equivalent of 0.08 ETH
...

    {
        ...
        // Check that new cat releases still available

        require(numberOfCatsAdded > 0);
        // Require release to have more than 0 cats

        currentReleaseCeiling = currentReleaseCeiling + numberOfCatsAdded;
        // Add new cats to release ceiling
        ...
        // Update price for new release of cats

        releaseCatIndexUpperBound.push(_totalSupply);
        // Track upper bound of cat index for this release



        ReleaseUpdate(numberOfCatsAdded, _totalSupply, catPrice, newImageHash);
        // Send EVM event containing details of release

        return _totalSupply;
        // Return new total supply of cats
...

    {
        ...
        // Check that release is id valid

        catReleaseToPrice[_releaseId] = catPrice;
        // Update price for cat release

        UpdateReleasePrice(_releaseId, catPrice);
        // Send EVM event with release id and price details
...

    {
        ...
        for (uint256 catIndex = startIndex; catIndex <= endIndex; catIndex++) {// Loop through cat index based on start/end index
            ...
            // Retrieve owner address from previous contract
        ...
        // Update count of total cats remaining to assign from prev contract
    ...
    function setCatAttributeValue(uint catIndex, uint attrIndex, string attrValue) {
        ...
        // cat index requested should not exceed total supply

        require(catIndexToAddress[catIndex] == msg.sender);
        // require sender to be cat owner

        require(attrIndex >= 0 && attrIndex < 6);
        // require that attribute index is 0 - 5

        bytes memory tempAttributeTypeText = bytes(attributeType[attrIndex]);

        require(tempAttributeTypeText.length != 0);
        // require that attribute being stored is not empty

        catAttributes[catIndex][attrIndex] = attrValue;
        // store attribute value string in contract based on cat index
    ...
    function transfer(address _to, uint256 _value) returns (bool success) {
        ...
        catIndexToAddress[_value] == msg.sender && // ensure sender is owner of cat
            ...
            // update (reduce) cat balance  from owner

            catIndexToAddress[_value] = _to;
            // set new owner of cat in cat index

            balanceOf[_to]++;
            // update (include) cat balance for recepient

            Transfer(msg.sender, _to, _value);
            // trigger event with transfer details to EVM

            success = true;
            // set success as true after transfer completed

        } else {

            success = false;
            // set success as false if conditions not met
    ...
    function balanceOf(address _owner) constant returns (uint256 balance) {
        ...
        // requires that cat owner balance is not 0

        return balanceOf[_owner];
        // return number of cats owned from array of balances by owner address
    ...
    function getCatPrice(uint catIndex) returns (uint catPrice) {
        ...
        require(catIndex < _totalSupply);
        // Require that cat index is valid
        ...
        if (catIndexToPriceException[catIndex] != 0) {// Check if there is any exception pricing
            ...
            // Return price if there is overriding exception pricing
        ...
        // Return cat price based on release pricing if no exception pricing
...

    {
        ...
        require(catPrice > 0);
        // Check that cat price is not 0

        catIndexToPriceException[catIndex] = catPrice;
        // Create cat price record in exception pricing array for this cat index
    ...
    function getCat(uint catIndex) payable {
        ...
        // require all cats have not been assigned/claimed

        require(catsRemainingToAssign != 0);
        // require cats remaining to be assigned count is not 0

        require(catIndexToAddress[catIndex] == 0x0);
        // require owner address for requested cat index is empty

        require(catIndex < _totalSupply);
        // require cat index requested does not exceed total supply
        ...
        // require ETH amount sent with tx is sufficient for cat price



        catIndexToAddress[catIndex] = msg.sender;
        // Assign sender's address as owner of cat

        balanceOf[msg.sender]++;
        // Increase sender's balance holder

        catsRemainingToAssign--;
        // Decrease cats remaining count

        pendingWithdrawals[owner] += msg.value;
        // Add paid amount to pending withdrawals for contract owner (bugfix in v3.0)

        Assign(msg.sender, catIndex);
        // Triggers address assignment event to EVM's
    ...
    function getCatOwner(uint256 catIndex) public returns (address) {
        ...
        // Return address at array position of cat index
    ...
    function catNoLongerForSale(uint catIndex) {
        ...
        require(catIndexToAddress[catIndex] == msg.sender);
        // Require that sender is cat owner

        require(catIndex < _totalSupply);
        // Require that cat index is valid

        catsForSale[catIndex] = Offer(false, catIndex, msg.sender, 0, 0x0);
        // Switch cat for sale flag to false and reset all other values

        CatNoLongerForSale(catIndex);
        // Create EVM event logging that cat is no longer for sale
    ...
    function offerCatForSale(uint catIndex, uint minSalePriceInWei) {
        ...
        // Set cat for sale flag to true and update with price details

        CatOffered(catIndex, minSalePriceInWei, 0x0);
        // Create EVM event to log details of cat sale
    ...
    function offerCatForSaleToAddress(uint catIndex, uint minSalePriceInWei, address toAddress) {
        ...
        // Set cat for sale flag to true and update with price details and only sell to address
    ...
    function buyCat(uint catIndex) payable {
        ...
        require(catIndex < _totalSupply);
        // require that cat index is valid and less than total cat index

        Offer offer = catsForSale[catIndex];

        require(offer.isForSale);
        // require that cat is marked for sale  // require buyer to have required address if indicated in offer

        require(msg.value >= offer.minPrice);
        // require buyer sent enough ETH

        require(offer.seller == catIndexToAddress[catIndex]);
        // require seller must still be owner of cat

        if (offer.sellOnlyTo != 0x0) {// if cat offer sell only to address is not blank

            require(offer.sellOnlyTo == msg.sender);
            // require that buyer is allowed to buy offer
        ...
        catIndexToAddress[catIndex] = msg.sender;
        // update cat owner address to buyer's address

        balanceOf[seller]--;
        // reduce cat balance of seller

        balanceOf[msg.sender]++;
        // increase cat balance of buyer

        Transfer(seller, msg.sender, 1);
        // create EVM event logging transfer of 1 cat from seller to owner



        CatNoLongerForSale(catIndex);
        // create EVM event logging cat is no longer for sale

        pendingWithdrawals[seller] += msg.value;
        // increase pending withdrawal amount of seller based on amount sent in buyer's message

        CatBought(catIndex, msg.value, seller, msg.sender);
        // create EVM event logging details of cat purchase
    ...
    function withdraw() {
        ...
        // store amount that can be withdrawn by sender

        pendingWithdrawals[msg.sender] = 0;
        // zero pending withdrawal amount

        msg.sender.transfer(amount);
        // before performing transfer to message sender
