...
contract MoonCatRescue {
    ...
    function rescueCat(bytes32 seed) activeMode returns (bytes5) {
        ...
        // cannot register any cats once supply limit is reached

        bytes32 catIdHash = keccak256(seed, searchSeed);
        // generate the prospective catIdHash

        require(catIdHash[0] | catIdHash[1] | catIdHash[2] == 0x0);
        // ensures the validity of the catIdHash

        bytes5 catId = bytes5((catIdHash & 0xffffffff) << 216);
        // one byte to indicate genesis, and the last 4 bytes of the catIdHash

        require(catOwners[catId] == 0x0);
        // if the cat is already registered, throw an error. All cats are unique.
    ...
    function nameCat(bytes5 catId, bytes32 catName) onlyCatOwner(catId) {
        ...
        // ensure the current name is empty; cats can only be named once

        require(!adoptionOffers[catId].exists);
        // cats cannot be named while they are up for adoption
    ...
    function acceptAdoptionOffer(bytes5 catId) payable {
        ...
        if (msg.value > offer.price) {
            ...
            // if the submitted amount exceeds the price allow the buyer to withdraw the difference
    ...
    function acceptAdoptionRequest(bytes5 catId) onlyCatOwner(catId) {
        ...
        // the adoption request must be cancelled before calling transferCat to prevent refunding the requester.
    ...
    function disableBeforeActivation() onlyOwner inactiveMode {
        ...
        mode = Modes.Disabled;
        // once the contract is disabled it's mode cannot be changed
    ...
    function activate() onlyOwner inactiveMode {
        ...
        // once the searchSeed is set it cannot be changed;

        mode = Modes.Live;
        // once the contract is activated it's mode cannot be changed
    ...
    function transferCat(bytes5 catId, address from, address to, uint price) private {
        ...
        adoptionOffers[catId] = AdoptionOffer(false, catId, 0x0, 0, 0x0);
        // cancel any existing adoption offer when cat is transferred



        AdoptionRequest storage request = adoptionRequests[catId];
        //if the recipient has a pending adoption request, cancel it
