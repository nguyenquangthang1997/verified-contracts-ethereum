...
contract AccountFactory is IAccountFactory, ACLTrait, ReentrancyGuard {
    ...
    configuratorOnly // T:[AF-13]
    {
        ...
        if (head == creditAccount) {
            ...
            head = _nextCreditAccount[head];
            // T:[AF-21] it exists cause we called _checkStock();
            ...
            _nextCreditAccount[prev] = _nextCreditAccount[creditAccount];
            // T:[AF-16]
            _nextCreditAccount[creditAccount] = address(0);
            // T:[AF-16]
        }
        ICreditAccount(creditAccount).connectTo(to, 0, 0);
        // T:[AF-16, 21]
        creditAccountsSet.remove(creditAccount);
        // T:[AF-16]
        emit TakeForever(creditAccount, to);
        // T:[AF-16, 21]
    }
    ...
    function mineCreditAccount() external nonReentrant {
        require(!isMiningFinished, Errors.AF_MINING_IS_FINISHED);
        // T:[AF-17]
        addCreditAccount();
        // T:[AF-18]
        ICreditAccount(tail).connectTo(address(this), 1, 1);
        // T:[AF-18]
    ...
    configuratorOnly // T:[AF-13]
    {
        ICreditAccount(account).cancelAllowance(token, targetContract);
        // T:[AF-20]
    }
    ...
    function isCreditAccount(address addr) external view returns (bool) {
        return creditAccountsSet.contains(addr);
        // T:[AF-16]
    }
}
