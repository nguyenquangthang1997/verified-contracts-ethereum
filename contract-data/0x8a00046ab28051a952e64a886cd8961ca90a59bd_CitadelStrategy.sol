...
contract CitadelStrategy is Ownable {
    ...
    function yield() external onlyVault {
        ...
        uint256[] memory _yieldAmts = new uint256[](4);
        // For emit yield amount of each farm
        // 1) Claim all rewards
        uint256 _yieldFees;
        // Curve HBTC/WBTC
    ...
    function _splitYieldFees(uint256 _amount) private {
        ...
        (bool _a,) = admin.call{value : _yieldFee}("");
        // 40%
        require(_a);
        (bool _t,) = communityWallet.call{value : _yieldFee}("");
        // 40%
        require(_t);
    ...
    function _investDAIETH(uint256 _amount) private {
        ...
        if (_amounts[1] > 0) {
            ...
            // 1389.083912192186144530 0.335765206816332767 17.202418926243352766
    ...
    function reimburse() external onlyVault {
        ...
        if (WETH.balanceOf(address(this)) < _amounts[1]) {// Balance of WETH > _amounts[1] when execute emergencyWithdraw()
            ...
            _withdrawCurve(_thirtyPercOfAmtWithdraw);
            // 30% from Curve HBTC/WBTC
            _withdrawPickleWBTC(_thirtyPercOfAmtWithdraw);
            // 30% from Pickle WBTC/ETH
            _withdrawSushiswap(_thirtyPercOfAmtWithdraw);
            // 30% from SushiSwap DPI/ETH
            _withdrawPickleDAI(_amounts[1].sub(_thirtyPercOfAmtWithdraw).sub(_thirtyPercOfAmtWithdraw).sub(_thirtyPercOfAmtWithdraw));
            // 10% from Pickle DAI/ETH
            _swapAllToETH();
            // Swap WBTC, DPI & DAI that get from withdrawal above to WETH
        }
    ...
    function _getLPTokenPrice() private view returns (uint256, uint256, uint256, uint256) {
        ...
        // Curve HBTC/WBTC
        uint256 _amountACurve = cPairs.balances(0);
        // HBTC, 18 decimals
        uint256 _amountBCurve = (cPairs.balances(1)).mul(1e10);
        // WBTC, 8 decimals to 18 decimals
