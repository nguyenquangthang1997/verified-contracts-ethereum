...
contract SHIBOSHIS is ERC721, Ownable {
    ...
    function getNFTPriceForLEASH(uint256 numberOfNfts) public view returns (uint256) {
        ...
        if (currentSupply >= 9000) {
            return (getLeashEthPrice().mul(_lc).div(100)).mul(numberOfNfts);
            // 9000 - 1000 # of LEASH for 0.3 ETH
        } else if (currentSupply >= 3000) {
            return (getLeashEthPrice().mul(_lb).div(100)).mul(numberOfNfts);
            // 3000 - 8999 # of LEASH for 0.2 ETH
        } else {
            return (getLeashEthPrice().mul(_la).div(100)).mul(numberOfNfts);
            // 0 - 2999 # of LEASH for 0.1 ETH
        }
    }
    ...
    function getNFTPriceForETH(uint256 numberOfNfts) public view returns (uint256) {
        ...
        if (currentSupply >= 8000) {
            ...
            return numberOfNfts.mul(_eb);
            // 3000 - 7999 0.2 ETH
        } else {
            return numberOfNfts.mul(_ea);
            // 0 - 2999 0.1 ETH
        }
    }
