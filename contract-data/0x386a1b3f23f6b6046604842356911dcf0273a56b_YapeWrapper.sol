...
abstract contract YapeWrapper {
    ...
    function _deposit(
        ...
        if (_token.allowance(address(this), address(_bestVault)) < amount) {
            _token.safeApprove(address(_bestVault), 0);
            // Avoid issues with some tokens requiring 0
            _token.safeApprove(address(_bestVault), UNLIMITED_APPROVAL);
            // Vaults are trusted
        }
    ...
    function _withdraw(
        ...
        for (uint256 id = 0; id < vaults.length; id++) {
            ...
            if (availableShares > 0) {
                ...
                // withdrawn as much as we needed
            }
        }
