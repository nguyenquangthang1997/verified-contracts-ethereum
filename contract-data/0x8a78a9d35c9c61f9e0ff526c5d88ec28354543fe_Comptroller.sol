...
contract Comptroller is ComptrollerV2Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function exitMarket(address cTokenAddress) external returns (uint) {
        ...
        if (storedList.length == 0) {
            ...
            // Set borrower index of moved item to correct index

            borrowerIndexes[msg.sender] = 0;
            // Reset sender borrower index to 0 for a gas refund

            borrowers[msg.sender] = false;
            // Tell the contract that the sender is no longer a borrower (so it knows to add the borrower back if they enter a market in the future)
    ...
    function getMaxRedeemOrBorrow(address account, CToken cTokenModify, bool isBorrow) internal returns (Error, uint) {
        ...
        if (shortfall > 0) return (Error.NO_ERROR, 0);
        // Shortfall, so no more borrow/redeem
    ...
    function _getMaxRedeemOrBorrow(uint liquidity, CToken cTokenModify, bool isBorrow) internal view returns (Error, uint) {
        ...
        if (liquidity <= 0) return (Error.NO_ERROR, 0);
        // No available account liquidity, so no more borrow/redeem
