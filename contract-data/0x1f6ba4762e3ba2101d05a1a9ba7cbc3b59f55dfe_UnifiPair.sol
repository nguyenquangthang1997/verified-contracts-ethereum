...
contract UnifiPair is IUnifiPair, UnifiERC20 {
    ...
    function initialize(address _token0, address _token1, address _wbnb) external {
        ...
        require(msg.sender == factory, 'Unifi: FORBIDDEN');
        // sufficient check
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(amount0In > 0 || amount1In > 0, 'Unifi: INSUFFICIENT_INPUT_AMOUNT');
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                if (pathToTrade.length > 0 && pathToTrade[pathToTrade.length - 1] == WBNB) {
                    ...
                    if (pd.upFees > 0) {
                        ...
                        IUnifiRouter(IUnifiFactory(factory).router()).swapExactTokensForETH(pd.upFees, 1, pathToTrade, address(pd.iUC), (block.timestamp).add(18000));
                        //get the WBNB back
        ...
        _update(pd.balance0, pd.balance1, _reserve0, _reserve1);
        //balance is wrong
