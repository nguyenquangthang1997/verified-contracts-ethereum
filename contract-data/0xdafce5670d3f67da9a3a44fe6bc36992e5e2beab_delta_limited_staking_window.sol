...
contract DELTA_Limited_Staking_Window {
    ...
    function _makeRefCode(address referrer) internal returns (uint256) {
        ...
        totalReferralIDs++;
        // lead to skip 0 code for the LSW
    ...
    function endLiquidityDeployment() public {
        ...
        require(block.timestamp > liquidityGenerationEndTimestamp.add(5 minutes), "LSW Not over yet.");
        // Added few blocks here
        ...
        wETH.transfer(DELTA_FINANCIAL_MULTISIG, balanceWETHPreSplit.div(2));
        // send half

        uint256 balanceWETHPostSplit = wETH.balanceOf(address(this));
        // requery
        ...
        deltaToken.transfer(reserveVaultAddress, deltaToken.balanceOf(address(this)));
        // queried again in case of rounding problems
    ...
    function handleReferredDepositWithAddress(address referrerAddress) internal returns (uint256) {
        ...
        if (referrerAddress == msg.sender) {return 0;}
        //We dont let self referrals and bail here without any bonuses.



        require(msg.value > 0, "Sanity check failure");

        uint256 wETHBonus = msg.value.div(20);
        // 5%

        uint256 creditBonus = wETHBonus;
        // Samesies
    ...
    function contributeLiquidity(bool readAndAgreedToLiquidityProviderAgreement, address referrerAddress, uint256 referralID) public payable {
        ...
        if (referralID != 0) {
            ...
            creditBonus = handleReferredDepositWithReferralID(referralID);
            // TO REVIEW: handleReferredDepositWithReferralID returns the reward (referrer and referee)

        } else if (referrerAddress != address(0)) {

            creditBonus = handleReferredDepositWithAddress(referrerAddress);
            // TO REVIEW: handleReferredDepositWithReferralID returns the reward (referrer and referee)
    ...
    function calculateCreditBonusBasedOnCurrentTime(uint256 depositValue) internal view returns (uint256) {
        ...
        uint256 percentLeft = secondsLeft.mul(100).div(totalSeconds);
        // 24 hours before LSW end, we get 7 for percentLeft - highest value for this possible is 100 (by a bot)
