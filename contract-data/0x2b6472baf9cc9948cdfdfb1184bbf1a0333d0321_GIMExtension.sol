...
contract GIMExtension is BaseExtension {
    ...
    returns (address[] memory, uint256[] memory, uint256[] memory)
    {
        ...
        uint256 newCounter;
        // Count amount of components added to newComponents array to add new components to next index
        for (uint256 i = 0; i < rebalanceComponentsLength; i++) {
            ...
            if (isIn) {
                oldComponentsTargetUnits[index] = _targetUnits[i];
                // Use index in order to map to correct component in currentComponents array
            } else {
