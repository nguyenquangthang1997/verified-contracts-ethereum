...
library IdleHelp {
    ...
    returns (uint256 tokenPrice) {
        ...
        if (bestToken == cToken) {
            ...
            // 202487304197710837666727644 ->

        } else {

            price = iERC20(iToken).tokenPrice();
            // eg 1001495070730287403 -> 1iToken in wei = 1001495070730287403 Token

        }

        navPool = price.mul(poolSupply);
        // eg 43388429749999990000 in DAI

        tokenPrice = navPool.div(totalSupply);
        // idleToken price in token wei
    ...
    public {
        ...
        if (bestToken != address(0)) {
            ...
            if (bestToken == cToken) {
                ...
                // token are now in this contract
