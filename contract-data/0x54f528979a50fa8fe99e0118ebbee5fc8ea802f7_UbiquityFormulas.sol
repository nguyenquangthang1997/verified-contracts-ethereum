...
contract UbiquityFormulas {
    ...
    function durationMultiply(
        ...
        bytes16 m = _multiplier.fromUInt().div(unit);
        // 0.0001
    ...
    returns (uint256 _newMultiplier)
    {
        ...
        bytes16 c = uint256(105 * 1e16).fromUInt();
        // 1.05
        bytes16 u = uint256(1e18).fromUInt();
        // 1
        bytes16 a = u.add(u.sub(p).abs());
        // 1 + abs( 1 - P )

        _newMultiplier = m.mul(c).div(a).toUInt();
        // nM = M * C / A
