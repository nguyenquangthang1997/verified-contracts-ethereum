...
contract NestMiningV1 {
    ...
    ) external onlyGovernance
    {
        ...
        state.biteFeeRate = initParams.biteFeeRate;
        // 0.1%
        state.miningFeeRate = initParams.miningFeeRate;
        // 0.1% on testnet
        state.priceDurationBlock = initParams.priceDurationBlock;
        // 5 on testnet
        state.maxBiteNestedLevel = initParams.maxBiteNestedLevel;
        state.biteInflateFactor = initParams.biteInflateFactor;
        // 1 on testnet
        state.biteNestInflateFactor = initParams.biteNestInflateFactor;
        // 1 on testnet
    ...
    returns (uint256 ethAmount, uint256 tokenAmount, uint256 blockNum)
    {
        ...
        blockNum = blockNum + uint256(state.priceDurationBlock);
        // safe math
    }
    ...
    returns (uint128, uint128, int128, uint32)
    {
        ...
        return (p, pi.avgTokenAmount, v, pi.height + uint32(state.priceDurationBlock));
        // safe math
    }
    ...
    {
        uint256 _period = block.number.sub(MiningV1Data.MINING_GENESIS_BLOCK_NUMBER).div(MiningV1Data.MINING_NEST_YIELD_CUTBACK_PERIOD);
