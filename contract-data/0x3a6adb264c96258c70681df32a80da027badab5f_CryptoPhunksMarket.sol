...
contract CryptoPhunksMarket {
    ...
    function buyPhunk(uint phunkIndex) payable public {
        ...
        // phunk not actually for sale
        if (offer.onlySellTo != address(0x0) && offer.onlySellTo != msg.sender) revert();
        // phunk not supposed to be sold to this user
        if (msg.value < offer.minValue) revert();
        // Didn't send enough ETH
        address seller = offer.seller;
        if (seller != phunksContract.ownerOf(phunkIndex)) revert();
        // Seller no longer owner of phunk
