...
library WadMath {
    ...
    function wadDivUp(uint x, uint y) internal pure returns (uint) {
        ...
        return ((x.mul(WAD)).add(y - 1)).div(y);
        // Can use "-" instead of sub() since div(y) will catch y = 0 case anyway
...
abstract contract Oracle {
    ...
    function cacheLatestPrice() public virtual returns (uint price) {
        ...
        price = latestPrice();
        // Default implementation doesn't do any cacheing, just returns price.  But override as needed
...
abstract contract USMTemplate is IUSM, Oracle, ERC20Permit, Delegable {
...

    {
        ...
        uint ethInPool = rawEthInPool.sub(msg.value);
        // Backing out the ETH just received, which our calculations should ignore
    ...
    function _updateMinFumBuyPrice(uint debtRatio_, uint ethInPool, uint fumTotalSupply) internal {
        ...
        if (debtRatio_ <= MAX_DEBT_RATIO) {// We've dropped below (or were already below, whatev) max debt ratio
            ...
            if (previous != 0) {
                ...
                minFumBuyPriceStored.timestamp = 0;
                // Clear mfbp
    ...
    function _updateBuySellAdjustment(uint oldDebtRatio, uint newDebtRatio, uint oldAdjustment) internal {
        ...
        if (oldDebtRatio != 0 && newDebtRatio != 0) {
            ...
            // Not nec the same as oldAdjustment, because of the time decay!
...

    {
        ...
        if (fumTotalSupply == 0) {
            ...
            return usmToEth(ethUsmPrice, WAD, upOrDown);
            // if no FUM issued yet, default fumPrice to 1 USD (in ETH terms)
...
contract ChainlinkOracle is Oracle {
    ...
    function latestChainlinkPrice() public view returns (uint price) {
        ...
        price = uint(rawPrice).mul(SCALE_FACTOR);
        // TODO: Cast safely
...
contract OurUniswapV2TWAPOracle is Oracle {
...

    {
        ...
        if (aAcceptable) {
            ...
            if (bAcceptable) {
                ...
                refPrice = newerStoredPrice;
                // Neither is *too* recent, so return the fresher of the two

            } else {

                refPrice = storedPriceA;
            ...
            refPrice = storedPriceB;
            // Only B is acceptable
...

    {
        ...
        farEnough = newTimestamp >= storedPrice.timestamp + MIN_TWAP_PERIOD;
        // No risk of overflow on a uint32
