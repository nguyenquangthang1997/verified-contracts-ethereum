...
contract ProofOfHumanity is IArbitrable, IEvidence {
    ...
    modifier onlyGovernor {require(msg.sender == governor, "The caller must be the governor");
        _;}
    ...
    function withdrawSubmission() external {
        ...
        withdrawFeesAndRewards(msg.sender, msg.sender, submission.requests.length - 1, 0, 0);
    ...
    function changeStateToPending(address _submissionID, address[] calldata _vouches, bytes[] calldata _signatures, uint[] calldata _expirationTimestamps) external {
        ...
        uint timeOffset = now - submissionDuration;
        // Precompute the offset before the loop for efficiency and then compare it with the submission time to check the expiration.
    ...
    function challengeRequest(address _submissionID, Reason _reason, address _duplicateID, string calldata _evidence) external payable {
...

        {
            ...
            if (currentReason != _reason) {
                ...
                uint8 reasonBit = 1 << (uint8(_reason) - 1);
        ...
        // Set this back to 0, since it's no longer relevant as the new round is created.
    ...
    function fundAppeal(address _submissionID, uint _challengeID, Party _side) external payable {
        ...
        // You can only fund either requester or challenger.
        ...
        if (round.paidFees[uint(_side)] >= totalCost) {
            ...
            if (round.sideFunded == Party.None) {
                ...
                round.sideFunded = Party.None;
                // Set this back to default in the past round as it's no longer relevant.
    ...
    function executeRequest(address _submissionID) external {
        ...
        withdrawFeesAndRewards(requester, _submissionID, requestID, 0, 0);
        // Automatically withdraw for the requester.
