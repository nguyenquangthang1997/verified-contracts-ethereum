...
contract Rho is RhoInterface, Math {
	...
	*/
    function openInternal(bool userPayingFixed, uint notionalAmount, uint fixedRateLimitMantissa) internal returns (bytes32 swapHash) {
        ...
        CTokenAmount memory supplierLiquidityTemp = supplierLiquidity;
        // copy to memory for gas
    ...
    function close(
        ...
        if (lateBlocks > CLOSE_GRACE_PERIOD_BLOCKS) {
            ...
            penaltyPercent = ONE_EXP.mantissa > penaltyPercent.mantissa ? penaltyPercent : ONE_EXP;
            // maximum of 100% penalty
    ...
    function closePayFixedSwapInternal(
        ...
        userPayout = subToZero_(add_(userCollateral, floatLeg), fixedLeg);
        // no underflows
