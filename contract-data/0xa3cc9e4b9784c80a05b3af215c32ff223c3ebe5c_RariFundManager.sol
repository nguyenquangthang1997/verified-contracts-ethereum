...
contract RariFundManager is Initializable, Ownable {
    ...
    function getRawFundBalance() public returns (uint256) {
        ...
        // ETH balance in fund controller contract
    ...
    function _depositTo(address to, uint256 amount) internal fundEnabled returns (bool) {
        ...
        uint256 fundBalance = reptTotalSupply > 0 ? getFundBalance() : 0;
        // Only set if used
        ...
        (bool success,) = _rariFundControllerContract.call.value(amount)("");
        // Transfer ETH to RariFundController
    ...
    function getREPTBurnAmount(address from, uint256 amount) internal returns (uint256) {
        ...
        uint256 fundBalance = getFundBalance() + address(this).balance;
        // accounts for funds currently inside RariFundManager

        require(fundBalance > 0, "Fund balance is zero.");

        uint256 reptAmount = amount.mul(reptTotalSupply).div(fundBalance);
        // check again
    ...
    function _withdrawFrom(address from, uint256 amount) internal fundEnabled cachePoolBalance returns (bool) {
        ...
        (bool senderSuccess,) = msg.sender.call.value(amount)("");
        // Transfer 'amount' in ETH to the sender
        ...
        if (balance > 0) {
            ...
            (bool controllerSuccess,) = _rariFundControllerContract.call.value(balance)("");
            // Transfer any leftover eth back to fund controller
