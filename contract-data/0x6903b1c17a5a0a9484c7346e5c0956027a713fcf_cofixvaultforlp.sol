...
contract CoFiXVaultForLP is ICoFiXVaultForLP, ReentrancyGuard {
    ...
    constructor(address cofi, address _factory) public {
        ...
        // https://etherscan.io/block/countdown/11040688
    }
    ...
    function addPool(address pool) external override onlyGovernance {
        ...
        // staking token is CoFiXPair (XToken)
        allPools.push(pool);
        // add once never delete, using for track
    ...
    function getPendingRewardOfLP(address pair) external override view returns (uint256) {
        ...
        if (poolState == POOL_STATE.INVALID || poolState == POOL_STATE.DISABLED) {
            return 0;
            // if pool is disabled, it can't mint by call distributeReward, so don't count on any reward for it
        }
    ...
    function distributeReward(address to, uint256 amount) external override nonReentrant {
        ...
        if (poolState == POOL_STATE.DISABLED) {
            return;
            // make sure tx would revert because user still want to withdraw and getReward
        }
    ...
    function currentCoFiRate() public override view returns (uint256) {
        ...
        if (periodIdx > 4) {
            periodIdx = 4;
            // after 5 years, the rate keep constant
        }
