...
library PoolLib {
    ...
    */
    ...
    )
    {
        ...
        IBPool bPool = IBPool(stakeAsset);
        // stakeAsset = Balancer Pool Tokens
        ...
        // Update StakeLockerFDT loss accounting for BPTs
    }
    ...
    */
    ...
    {
        poolDelegatePortion = claimInfo[1].mul(delegateFee).div(10_000).add(claimInfo[3]);
        // Pool Delegate portion of interest plus fee.
        stakeLockerPortion = claimInfo[1].mul(stakingFee).div(10_000);
        // StakeLocker portion of interest.

        principalClaim = claimInfo[2].add(claimInfo[4]).add(claimInfo[5]);
        // principal + excess + amountRecovered
        interestClaim = claimInfo[1].sub(claimInfo[1].mul(delegateFee).div(10_000)).sub(stakeLockerPortion);
        // leftover interest
    }
    ...
    */
    function _bdiv(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c0 = a * WAD;
        require(a == 0 || c0 / a == WAD, "P:DIV_INTERNAL");
        // bmul overflow
        uint256 c1 = c0 + (b / 2);
        require(c1 >= c0, "P:DIV_INTERNAL");
        //  badd require
        return c1 / b;
    }
    ...
    */
    function _convertFromUsd(IMapleGlobals globals, address liquidityAsset, uint256 usdAmount) internal view returns (uint256) {
        ...
        .div(globals.getLatestPrice(liquidityAsset));
        // Convert to Liquidity Asset value.
    }
}
...
contract Pool is PoolFDT {
    ...
    */
    function claim(address loan, address dlFactory) external returns (uint256[7] memory claimInfo) {
        ...
        _transferLiquidityAsset(poolDelegate, poolDelegatePortion);
        // Transfer the fee and portion of interest to the Pool Delegate.
        _transferLiquidityAsset(stakeLocker, stakeLockerPortion);
        // Transfer the portion of interest to the StakeLocker.
    ...
    */
    function _handleDefault(address loan, uint256 defaultSuffered) internal {
        ...
        // Subtract rest of the Loan's principal from `principalOut`.
    ...
    */
    function deposit(uint256 amt) external {
        ...
        // Reset the LP's withdraw cooldown if they had previously intended to withdraw.
    ...
    */
    function _canWithdraw(address account, uint256 wad) internal view {
        ...
        // Restrict withdrawal during lockup period
        require(balanceOf(account).sub(wad) >= totalCustodyAllowance[account], "P:INSUF_TRANS_BAL");
        // Account can only withdraw tokens that aren't custodied
    }
    ...
    */
    function _transfer(address from, address to, uint256 wad) internal override {
        ...
        require(block.timestamp > (withdrawCooldown[to] + lpCooldownPeriod + lpWithdrawWindow), "P:TO_NOT_ALLOWED");
        // Recipient must not be currently withdrawing.
        require(recognizableLossesOf(from) == uint256(0), "P:RECOG_LOSSES");
        // If an LP has unrecognized losses, they must recognize losses using `withdraw`.
