...
contract BridgePool is MultiCaller, Testable, BridgePoolInterface, ERC20, Lockable {
    ...
    constructor(
        ...
        // Fetch OptimisticOracle and Store addresses and L1Token finalFee.
        syncWithBridgeAdminParams();
        // Fetch ProposerBondPct OptimisticOracleLiveness, Identifier from the BridgeAdmin.
    }
    ...
    function relayAndSpeedUp(DepositData memory depositData, uint64 realizedLpFeePct) public nonReentrant() {
        ...
        // Book off maximum liquidity used by this relay in the pending reserves.
    ...
    function _liquidityUtilizationPostRelay(uint256 relayedAmount) internal returns (uint256) {
        _sync();
        // Fetch any balance changes due to token bridging finalization and factor them in.
    ...
    function _exchangeRateCurrent() internal returns (uint256) {
        ...
        // initial rate is 1 pre any mint action.

        // First, update fee counters and local accounting of finalized transfers from L2 -> L1.
        _updateAccumulatedLpFees();
        // Accumulate all allocated fees from the last time this method was called.
        _sync();
        // Fetch any balance changes due to token bridging finalization and factor them in.
