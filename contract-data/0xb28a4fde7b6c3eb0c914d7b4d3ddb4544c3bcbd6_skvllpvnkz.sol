...
contract Skvllpvnkz is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    ** starting after the initial reserved token pools */
    ...
    function withdrawAll() external payable onlyOwner {
        ...
        require(payable(t1).send(_each * 31));
        // TM1

        require(payable(t2).send(_each * 21));
        // TM2

        require(payable(t3).send(_each * 21));
        // TM3

        require(payable(t4).send(_each * 21));
        // TM4

        require(payable(t5).send(_each * 6));
        // Comm
