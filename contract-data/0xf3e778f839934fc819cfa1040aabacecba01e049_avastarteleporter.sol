...
contract TraitFactory is AvastarState {
...

    {
        ...
        for (uint8 slot = 0; slot <= uint8(Gene.HAIR_STYLE); slot++) {
            ...
            slotMultiplier = uint256(slotConst ** slot);
            // Create slot multiplier

            bitMask = slotMask * slotMultiplier;
            // Create bit mask for slot

            slottedValue = _traitHash & bitMask;
            // Extract slotted value from hash
