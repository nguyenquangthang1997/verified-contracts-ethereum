...
contract CleverGirls is ERC721Enumerable, Ownable {
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(a1).send(percent * 43));
        // 43% for Fat Baby <3
        require(payable(a2).send(percent * 24));
        // 24% for Steamed Bun
        require(payable(a3).send(percent * 24));
        // 24% for SINCOS
        require(payable(a4).send(percent * 5));
        //  5% for the collection wallet
        require(payable(a5).send(percent * 4));
        //  4% for ghost kid for writing and helping
    }
}