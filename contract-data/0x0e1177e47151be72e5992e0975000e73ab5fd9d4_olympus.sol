...
contract AaveAllocator is Ownable {
    ...
    function deposit(address token, uint amount) public onlyPolicy() {
        ...
        require(!exceedsLimit(token, amount));
        // ensure deposit is within bounds
        ...
        if (depositToTreasury) {// if aTokens are being deposited into treasury
            ...
            address aToken = aTokenRegistry[token];
            // address of aToken

            uint aBalance = IERC20(aToken).balanceOf(address(this));
            // balance of aToken received



            IERC20(aToken).approve(address(treasury), aBalance);
            // approve to deposit aToken into treasury

            treasury.deposit(aBalance, aToken, value);
            // deposit using value as profit so no OHM is minted
    ...
    function withdraw(address token, uint amount) public onlyPolicy() {
        ...
        if (depositToTreasury) {// if aTokens are being deposited into treasury
            ...
            treasury.manage(aToken, amount);
            // retrieve aToken from treasury

        }


        IERC20(aToken).approve(address(lendingPool), amount);
        // approve to withdraw from lending pool

        lendingPool.withdraw(token, amount, address(this));
        // withdraw from lending pool, returning asset



        uint balance = IERC20(token).balanceOf(address(this));
        // balance of asset received from lending pool
        ...
        // approve to deposit asset into treasury
    ...
    function addToken(address token, address aToken, uint max) external onlyPolicy() {
        ...
        require(aTokenRegistry[token] == address(0));
        // cannot add token twice



        aTokenRegistry[token] = aToken;
        // maps token to aToken

        aTokens.push(aToken);
        // tracks aToken in array

        deployLimitFor[token] = max;
        // sets max token can be deployed
    ...
    function lowerLimit(address token, uint newMax) external onlyPolicy() {
        ...
        // cannot set limit below what has been deployed already
    ...
    function enableDepositToTreasury() external onlyPolicy() {
        ...
        harvest();
        // claim accrued rewards to this address first
        ...
        for (uint i = 0; i < aTokens.length; i++) {
            ...
            if (balance > 0) {
                ...
                IERC20(aToken).approve(address(treasury), balance);
                // approve to deposit asset into treasury

                treasury.deposit(balance, aToken, value);
                // deposit using value as profit so no OHM is minted
    ...
    function revertDepositToTreasury() external onlyPolicy() {
        ...
        // future aToken deposits will be held in this contract
    ...
    function accountingFor(address token, uint amount, uint value, bool add) internal {
        ...
        if (add) {
            ...
            deployedFor[token] = deployedFor[token].add(amount);
