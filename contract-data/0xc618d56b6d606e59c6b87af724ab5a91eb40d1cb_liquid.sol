...

{
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        uint BurnBP = fromBurnBP;
        // BurnBP = min(fromBurnBP, toBurnBP)
...

    {
        ...
        if (gContractWhitelist[user] < 2) {// 0: disableALL; 1: disableBUY; 2: disableSELL; 3: allowALL;
            ...
            if (gContractCheckBuyLevel % 2 == 1) require(!isContract(user), '!human');
            // 0: no check; 1: methodA; 2: methodB; 3: both;

            if (gContractCheckBuyLevel >= 2) require(user == tx.origin, '!human');
            // 0: no check; 1: methodA; 2: methodB; 3: both;

        }


        require((MARKET_OPEN_STAGE > 0) || (user == govAddr), '!market');
        // govAddr can test contract before market open
...

    {
        ...
        if (gContractWhitelist[user] % 2 == 0) {// 0: disableALL; 1: disableBUY; 2: disableSELL; 3: allowALL;
            ...
            if (gContractCheckSellLevel % 2 == 1) require(!isContract(user), '!human');
            // 0: no check; 1: methodA; 2: methodB; 3: both;

            if (gContractCheckSellLevel >= 2) require(user == tx.origin, '!human');
            // 0: no check; 1: methodA; 2: methodB; 3: both;
