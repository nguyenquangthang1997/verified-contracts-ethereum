...
contract Auth is DSNote {
    ...
    modifier auth {require(wards[msg.sender] == 1);
        _;}
}
...
contract Title is Auth, ERC721Metadata {
    ...
    function _issue(address usr) internal returns (uint) {
        _mint(usr, count);
        count += 1;
        // can't overflow, not enough gas in the world to pay for 2**256 nfts.
        return count - 1;
    }
...
contract TitleOwned {
    ...
    modifier owner (uint loan) {require(title.ownerOf(loan) == msg.sender);
        _;}
}
