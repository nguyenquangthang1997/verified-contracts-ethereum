...
contract Arbitrable is IArbitrable {
    ...
    modifier onlyArbitrator {require(msg.sender == address(arbitrator), "Can only be called by the arbitrator.");
        _;}
...
contract KlerosLiquid is TokenController, Arbitrator {
    ...
    modifier onlyDuringPhase(Phase _phase) {require(phase == _phase);
        _;}
    ...
    modifier onlyDuringPeriod(uint _disputeID, Period _period) {require(disputes[_disputeID].period == _period);
        _;}
    ...
    modifier onlyByGovernor() {require(governor == msg.sender);
        _;}
    ...
    function executeGovernorProposal(address _destination, uint _amount, bytes _data) external onlyByGovernor {
        ...
        // solium-disable-line security/no-call-value
    ...
    function _setStake(address _account, uint96 _subcourtID, uint128 _stake) internal returns (bool succeeded) {
        ...
        if (!(_stake == 0 || currentStake > 0 || juror.subcourtIDs.length < MAX_STAKE_PATHS))
            ...
            return false;
        // Maximum stake paths reached.

        uint newTotalStake = juror.stakedTokens - currentStake + _stake;
        // Can't overflow because _stake is a uint128.
