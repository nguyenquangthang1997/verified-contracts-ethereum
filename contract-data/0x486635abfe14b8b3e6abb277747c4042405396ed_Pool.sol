...
contract Pool is IPool, ERC20Permit {
    ...
    returns (uint128 retrieved)
    {
        ...
        // Cache can never be above balances
    ...
    returns (uint256, uint256, uint256)
    {
        ...
        if (supply == 0) {
            ...
            tokensMinted = baseIn;
            // If supply == 0 we are initializing the pool and tokensMinted == baseIn; fyTokenIn == 0
        } else {
    ...
    returns (uint256 tokensBurned, uint256 tokenOut, uint256 fyTokenOut)
    {
        ...
        // use the real balance rather than the virtual one
