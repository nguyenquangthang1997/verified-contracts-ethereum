...
contract Corn is ERC777, IERC777Recipient, ReentrancyGuard {
    ...
    requireFarm(_msgSender(), false) // Ensure LAND is not already in a farm
    {
        ...
        senderFarm.lastHarvestedBlockNumber = block.number;
        // Reset the last harvest height to the new LAND allocation height
    ...
    requireFarm(targetAddress, true) // Ensure the address you are composting to has a farm on the LAND
    {
        ...
        Farm storage targetFarm = farms[targetAddress];
        // Shortcut accessor, pay attention to targetAddress here
    ...
    requireFarm(sourceAddress, true) // Ensure the adress that is being harvested has a farm on the LAND
    {
        ...
        Farm storage sourceFarm = farms[sourceAddress];
        // Shortcut accessor, pay attention to sourceAddress here
    ...
    returns (uint256)
    {
        ...
        targetFarm.lastHarvestedBlockNumber.add(maxGrowthCycle);
        // end of growth cycle last allowed block
        ...
        uint256 amount = targetFarm.amount;
        // Total of size of the farm in LAND for this address
    ...
    returns (uint256)
    {
        ...
        uint256 timeMultiplier = Math.min(maxMaturityBoost, blockDiff);
        // Min 1x, Max 3x
