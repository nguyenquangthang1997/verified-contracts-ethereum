...
contract ShortCERC20Order is CompoundOrder {
    modifier isValidPrice(uint256 _minPrice, uint256 _maxPrice) {
        ...
        // Get the shorting token's price in USD
        require(tokenPrice > 0);
        // Ensure asset exists on Compound
    ...
    isValidPrice(_minPrice, _maxPrice)
    {
        ...
        usdc.safeTransferFrom(owner(), address(this), collateralAmountInUSDC);
        // Transfer USDC from PeakDeFiFund
        ...
        // Clear USDC allowance of Compound USDC market
        usdc.safeApprove(address(CUSDC), collateralAmountInUSDC);
        // Approve USDC transfer to Compound USDC market
        require(CUSDC.mint(collateralAmountInUSDC) == 0);
        // Transfer USDC into Compound as supply
        usdc.safeApprove(address(CUSDC), 0);
        require(market.borrow(loanAmountInToken) == 0);
        // Take out loan
        (bool negLiquidity,) = getCurrentLiquidityInUSDC();
        require(!negLiquidity);
        // Ensure account liquidity is positive

        // Convert loaned tokens to USDC
        (uint256 actualUSDCAmount,) = __sellTokenForUSDC(loanAmountInToken);
        loanAmountInUSDC = actualUSDCAmount;
        // Change loan amount to actual USDC received
    ...
    {
        require(buyTime > 0);
        // Ensure the order has been executed
