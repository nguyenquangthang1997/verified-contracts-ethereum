...
contract ScdMcdMigration {
    ...
    function migrate(
        ...
        // CDP locked collateral

        uint ethAmt = tub.bid(pethAmt);
        // CDP locked collateral equiv in ETH
        ...
        tub.shut(cup);
        // CDP is closed using the SAI just exited and the MKR previously sent by the user (via the proxy call)

        tub.exit(pethAmt);
        // Converts PETH to WETH
...
contract MigrationProxyActions is DSMath {
    ...
    function migratePayFeeWithDebt(
        ...
        if (ok && val != 0) {
            ...
            // 1 extra wei MKR to avoid any possible rounding issue after drawing new SAI
