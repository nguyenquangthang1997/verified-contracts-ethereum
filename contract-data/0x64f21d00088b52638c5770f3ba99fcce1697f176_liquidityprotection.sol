...
library MathEx {
    ...
    function accurateRatio(
        ...
        if (_a > maxVal) {
            ...
            _a /= c;
            // we can now safely compute `_a * _scale`
            _b /= c;
        }
        if (_a != _b) {
            uint256 n = _a * _scale;
            ...
            if (d >= _a) {
                ...
                // we can now safely compute `_scale - x`
                uint256 y = _scale - x;
                return (x, y);
            }
            if (n < _b - (_b - _a) / 2) {
                return (0, _scale);
                // `_a * _scale < (_a + _b) / 2 < MAX_UINT256 < _a + _b`
            }
            return (1, _scale - 1);
            // `(_a + _b) / 2 < _a * _scale < MAX_UINT256 < _a + _b`
        }
...
contract LiquidityProtection is ILiquidityProtection, TokenHandler, Utils, Owned, ReentrancyGuard, Time {
    ...
    function addBaseTokenLiquidity(
        ...
        systemStore.incSystemBalance(poolToken, poolTokenAmount - poolTokenAmount / 2);
