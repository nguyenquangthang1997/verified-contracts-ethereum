...
contract CoreUtils {
    ...
    function CollateralDecimal(uint256 id) public view returns (uint256) {
        ...
        if (b.collateralToken() == address(0)) return 18;
        //ETH
    ...
    function pawnBalanceInUsd(uint256 id) public view returns (uint256) {
        ...
        uint256 pawnUsd = pawnPrice(id).mul(b.getBorrowAmountGive()).div(unitPawn);
        //1e18
    ...
    function X(uint256 id) public view returns (uint256 res) {
        ...
        uint256 pcrXdis = b.depositMultiple().mul(b.discount()).div(_1);
        //1e18
        ...
        uint256 denominator = price.mul(pcrXdis).div(_1);
        //1e18
        ...
        res = gap.mul(unitPawn).div(denominator);
        //1e18/1e18*1e18 == 1e18
    ...
    function getLiquidateAmount(uint id, uint y1) external view returns (uint256, uint256) {
        ...
        if (y <= b.partialLiquidateAmount()) {
            ...
            require(y1 >= decUnit, "below min liquidate amount");
            //设置最小清算额度为1单位
