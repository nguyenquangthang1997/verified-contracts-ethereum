...
contract FAANGStrategy is Ownable {
    ...
    constructor(
        ...
        IERC20(0x87dA823B6fC8EB8575a235A824690fda94674c88).approve(_mirustPool, type(uint).max);
        //approve mirUST uniswap LP token to stake on mirror
    ...
    function yield() external onlyVault {
        ...
        if (totalEarnedMIR > 0) {
            ...
            uint _yieldFee = totalEarnedMIR.div(10);
            //10%
            uint _splitFee = _yieldFee.mul(2).div(5);
            mir.safeTransfer(treasuryWallet, _splitFee);
            //4 %
            mir.safeTransfer(communityWallet, _splitFee);
            //4 %
            mir.safeTransfer(strategist, _yieldFee.sub(_splitFee).sub(_splitFee));
            //2 %
