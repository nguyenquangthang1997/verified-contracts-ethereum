...
contract xInvCore is Exponential, TokenErrorReporter {
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        /* we move delegates */
        _moveDelegates(address(0), delegates[minter], uint96(vars.mintTokens));
        // NOTE: Check for potential overflows due to conversion from uint256 to uint96
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn, bool useEscrow) internal returns (uint) {
        ...
        /* We move delegates */
        _moveDelegates(delegates[redeemer], address(0), uint96(vars.redeemTokens));
        // NOTE: Check for potential overflows due to conversion from uint256 to uint96
    ...
    function _setRewardTreasury(address newRewardTreasury) public returns (uint) {
        ...
        rewardTreasury = newRewardTreasury;
        // it's acceptable to set it as address(0)
    ...
    function _setRewardPerBlock(uint newRewardPerBlock) public returns (uint) {
        ...
        // it's acceptable to set it as 0
